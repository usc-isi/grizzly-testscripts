...
"
	...
	for j in $TENANT; do
		...
		if [ $j == $TENANT1 ]; then
		    echo " "
		    echo "---------------------------------------------------------------------------"
		    echo " 8. ssh -i $KEY $USER@$INST_IP"
		    echo "---------------------------------------------------------------------------"
		    for k in `seq 1 $TIMEOUT`; do
			ssh -o "StrictHostKeyChecking no" -o "BatchMode yes" -i $KEY $USER@$INST_IP ls > /dev/null
  			if [ $? -eq 0 ]; then
		...
		if [ $j == $TENANT2 ]; then
		    ...
		    echo " "
		    echo "---------------------------------------------------------------------------"
		    echo " 9. ssh -i $KEY $USER@$OTHER_INST_IP"
		    echo "---------------------------------------------------------------------------"
		    ssh -o "StrictHostKeyChecking no" -o "BatchMode yes" -i $KEY $USER@$OTHER_INST_IP ls > /dev/null
		    if [ $? -eq 0 ]; then
